...
A lo largo del documento se describirán los conceptos de imágenes del contenedor y capas de imagen del contenedor. Para obtener más información sobre las imágenes y las capas de imagen, vea [Administración de imágenes del contenedor de Windows](../management/manage_images.md). 

Para obtener información detallada sobre los archivos Dockerfile, consulte la [referencia sobre Dockerfile en docker.com]( https://docs.docker.com/engine/reference/builder/).
...
Para obtener más ejemplos de archivos Dockerfile para Windows, consulte el [Repositorio de Dockerfile para Windows] (https://github.com/Microsoft/Virtualization-Documentation/tree/master/windows-container-samples).
...
En este ejemplo se instala el Paquete de desarrollador de .NET Framework 4.5.2, para lo cual se extrae primero y, después, se inicia el programa de instalación real. 
```none
...
PowerShell y el comando `Invoke-WebRequest` pueden resultar útiles al recopilar información o archivos de un servicio web. Por ejemplo, si está creando una imagen que incluye Python, podría usar el ejemplo siguiente.
...
> Invoke-WebRequest no se admite actualmente en Nano Server.
...
[Optimización de los archivos Dockerfile y la compilación de Docker para Windows] (./optimize_windows_dockerfile.md)
