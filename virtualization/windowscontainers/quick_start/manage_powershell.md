...
En este tutorial se detallan los contenedores de Windows Server y Hyper-V. Cada tipo de contenedor tiene sus propios requisitos básicos. Con la documentación de contenedores de Windows se incluye un procedimiento para implementar rápidamente un host de contenedor. Se trata de la manera más fácil para empezar a trabajar rápidamente con contenedores de Windows. Si aún no dispone de un host de contenedor, consulte el [Inicio rápido de implementación del host de contenedor](./container_setup.md).
...
Para validar que la imagen del sistema operativo Windows Server Core se instaló, use el comando `Get-ContainerImage`. Puede ver varias imágenes de sistema operativo, lo cual es normal.
...
Para crear un contenedor de Windows Server, use el comando `New-Container`. En el ejemplo siguiente se crea un contenedor denominado `TP4Demo` a partir de la imagen de sistema operativo `WindowsServerCore` y se conecta el contenedor a un conmutador de máquina virtual denominado `Virtual Switch`. Tenga en cuenta que la salida, un objeto que representa el contenedor, se almacena en una variable `$con`. Esta variable se usa en los comandos posteriores.
...
Conéctese con el contenedor mediante el comando `Enter-PSSession`. Observe que cuando se creó la sesión de PowerShell con el contenedor, el símbolo de PowerShell cambia para reflejar el nombre del contenedor.
...
Cuando finalice la instalación de IIS, salga del contenedor escribiendo `exit`. Esto devuelve la sesión de PowerShell a la del host de contenedor.
...
Cree un contenedor nuevo, esta vez a partir de la imagen de contenedor `WindowsServerCoreIIS`.
...
Cuando el contenedor se detenga, se puede quitar con el comando `Remove-Container`.

```powershell
PS C:\> Remove-Container -Name IIS -Force
```

Por último, se puede quitar una imagen de contenedor con el comando `Remove-ContainerImage`.
...
En el momento de la publicación de TP4, los contenedores de Hyper-V deben utilizar una imagen de sistema operativo Nano Server Core. Para validar que se instaló la imagen de sistema operativo de Nano Server, use el comando `Get-ContainerImage`.
...
Utilice el comando `Add-ContainerSharedFolder` para crear una nueva carpeta compartida en el nuevo contenedor.
...
```
Cuando se encuentre en la sesión remota, tenga en cuenta que la carpeta compartida `c:\iisinstall\en-us` se creó y, sin embargo, está vacía.
...
Como el contenedor ejecuta una imagen de sistema operativo Nano Server, se necesitan los paquetes de IIS de Nano Server para instalar IIS. Puede encontrarlos en los medios de instalación de Windows Server 2016 TP4, en el directorio `NanoServer\Packages`.
...
Cuando se complete, el directorio `c:\share` en el host de contenedor debería estar configurado así.
